...
$errors = [];
$f = ['name'=>'','email'=>'','phone'=>'','company'=>'','service_interest'=>'','budget'=>'','message'=>'','source'=>'walk-in','status'=>'new','notes'=>''];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $pdo) {
    ...
    if (empty($errors)) {
        ...
        $stmt = $pdo->prepare(
            "INSERT INTO leads (ref,name,email,phone,company,service_interest,budget,message,status,source,notes)
             VALUES (?,?,?,?,?,?,?,?,?,?,?)"
        );
        $stmt->execute([
            $ref, $f['name'], $f['email']?:null, $f['phone']?:null, $f['company']?:null,
            $f['service_interest']?:null, $f['budget']?:null, $f['message']?:null,
            $f['status'], $f['source'], $f['notes']?:null,
        ]);
...
<div class="max-w-2xl">
  ...
  <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
    <div class="px-6 py-5 border-b border-gray-100">
      <h2 class="font-semibold text-gray-800">New Lead</h2>
      <p class="text-sm text-gray-400 mt-0.5">Manually add a lead to the pipeline</p>
    </div>
    ...
    <form method="POST" class="px-6 py-5 space-y-5">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div>
          ...
          <select name="source" class="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-amber-400">
            <?php foreach (['website'=>'Website','walk-in'=>'Walk-in','referral'=>'Referral','phone'=>'Phone Call','email'=>'Email','social'=>'Social Media'] as $v=>$l): ?>
        ...
        <div>
          ...
          <select name="status" class="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-amber-400">
            <?php foreach (['new'=>'New','contacted'=>'Contacted','qualified'=>'Qualified'] as $v=>$l): ?>
      ...
      <div class="flex items-center gap-3 pt-2">
        ...
        <a href="/crm/leads/" class="text-sm text-gray-400 hover:text-gray-600">Cancel</a>
      </div>
    </form>
  </div>
</div>
